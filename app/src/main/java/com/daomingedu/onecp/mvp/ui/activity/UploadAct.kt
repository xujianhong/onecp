...
import com.daomingedu.onecp.mvp.ui.activity.presenter.IUploadPresenter
import com.daomingedu.onecp.mvp.ui.activity.presenter.UploadCompl
import com.daomingedu.onecp.mvp.ui.activity.view.IUploadView
import com.daomingedu.onecp.util.*
...
class UploadAct : AppCompatActivity(), IUploadView {
        ...
        MyOkGoUtil.postGetString(this, Api.APP_DOMAIN + "/api/common/getWaterMark", httpParams, object : Handler(){
            override fun handleMessage(msg: Message) {
                ...
                if (msg.what == 0){
                    ...
                    if (saveBitmapToSdCard(this@UploadAct, bitmap, "test")){
                        when(videoType){
                            1 -> {
                                when (MemoryUtil.memoryIsAvailble(this@UploadAct)){
                                    0 -> takeVideo("")
                                    1 -> {
                                        val dialog = AlertDialog.Builder(this@UploadAct)
                            ...
                            0 -> {
                                val builder = AlertDialog.Builder(this@UploadAct)
                                ...
                                builder.setNegativeButton("选择缓存视频", object : DialogInterface.OnClickListener{
                                    override fun onClick(dialog: DialogInterface?, which: Int) {
                                        dialog?.dismiss()
                                        UploadVideoListAct.startUploadVideoListActivity(this@UploadAct, 1, isShowImport)
                                    }
                                })
                                builder.create().show()
                            }
                    ...
                    Toast.makeText(this@UploadAct, "请检查网络连接是否正常", Toast.LENGTH_SHORT).show()
                }
            }
        })
    }
...
//        builder.maxFileSize(((application as MyApp).getVideo() / 1024 / 1024) as Int)
    ...
    companion object {
        ...
        fun startUploadActivity(activity: AppCompatActivity, id: String, url: String, isShowSongList: Int, isShowImport: Int){
            val intent = Intent(activity, UploadAct::class.java)
            intent.putExtra("id", id)
